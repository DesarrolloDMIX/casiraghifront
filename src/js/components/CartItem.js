...
const CartItem = function (element) {
  ...
  this.init = () => {
    ...
    this.qtyRemoveButton.addEventListener(
      "click",
      this.handleQtyRemove.bind(this)
    );
...
CartItem.prototype.handleQuantityUpdated = function (newQty) {
  ...
  let unitFraction = parseInt(
    priceElement.dataset.unitPriceFraction.replace(".", "")
  );
...
CartItem.prototype.handleQuantityPersistance = debounce(function (e) {
  ...
  fetch(
    `${location.protocol}//${location.host}/carrito/update-product-quantity`,
    {
      method: "POST",
      headers: {
