...
const Cart = function (element) {
    this.elem = element
    this.totalPriceFractionElement = this.elem.querySelector('.cart__bottom .price__fraction')
    this.totalPriceCentsElement = this.elem.querySelector('.cart__bottom .price__cents')
    ...
    this.init = () => {
        ...
        this.elem.addEventListener('quantityUpdated', this.handleQuantityUpdated.bind(this))
...
Cart.prototype.handleQuantityUpdated = function (evt) {
    let newQty = Array.from(this.cartItems).reduce((previus, current) => {
        return parseInt(previus) + parseInt(current.querySelector('.cart-item__qta-number').value)
    }, 0)
